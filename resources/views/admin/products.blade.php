...
@section('content')
            ...
            @foreach($products as $product)
                <tr class="border-b">
                    ...
                    <td class="px-4 py-2 flex space-x-2">
                        <button onclick="openEditModal({{ $product->id }})" class="bg-yellow-500 text-white px-4 py-2 rounded">Edit</button>
    ...
    <div id="createEditModal" class="hidden fixed top-0 left-0 w-full h-full bg-gray-800 bg-opacity-50 flex items-center justify-center">
        <div class="bg-white p-6 rounded shadow-lg w-1/2">
            ...
            <form id="createEditForm" action="{{ route('admin.products.create') }}" method="POST">
                @csrf
    ...
    <script>
        function openCreateModal() {
            ...
            document.getElementById('createEditForm').action = '{{ route('admin.products.create') }}';
        ...
        function openEditModal(id) {
            ...
            document.getElementById('createEditForm').action = `/admin/products/update/${id}`;
