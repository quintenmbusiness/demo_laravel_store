...
@section('content')
    <div class="container mx-auto p-6">
        ...
        <div class="mb-4">
            <button onclick="openCreateModal()" class="bg-blue-500 text-white px-4 py-2 rounded">Create New Order</button>
        </div>
        ...
        <table class="table-auto w-full bg-white shadow-md rounded">
            ...
            @foreach($orders as $order)
                <tr class="border-b">
                    ...
                    <td class="px-4 py-2">{{ $order->stock }}</td>
                    <td class="px-4 py-2">{{ $order->category_id }}</td>
                    <td class="px-4 py-2 flex space-x-2">
                        <button onclick="openEditModal({{ $order->id }})" class="bg-yellow-500 text-white px-4 py-2 rounded">Edit</button>
    ...
    <div id="createEditModal" class="hidden fixed top-0 left-0 w-full h-full bg-gray-800 bg-opacity-50 flex items-center justify-center">
        <div class="bg-white p-6 rounded shadow-lg w-1/2">
            <h2 id="modalTitle" class="text-xl font-bold mb-4">Create Order</h2>
            <form id="createEditForm" action="{{ route('admin.orders.create') }}" method="POST">
                @csrf
    ...
    <script>
        function openCreateModal() {
            document.getElementById('modalTitle').innerText = 'Create Order';
            document.getElementById('createEditForm').action = '{{ route('admin.orders.create') }}';
        ...
        function openEditModal(id) {
            ...
            document.getElementById('createEditForm').action = `/admin/orders/update/${id}`;
